...
public class OfferManagementViewModel implements ViewModel {
	...
	// Liste aller Angebote + Mitarbeiter + Kunden + A.Positionen (kann von
	// Datenbank hier Daten in Observ.List hinein geben)
	...
	// FilteredList erstellen für alle SuchMethoden (=gefilterte Tabelle nach
	// Suche)
	...
	public EventHandler<ActionEvent> FindByOfferNumber() {
		 ...
		 * controler.getOfferField().textProperty().addListener((observable,
		 * oldValue, newValue) -> { controler.filteredData.setPredicate(offer ->
		 * { // wenn nichts im Filter, dann alle angezeigt if (newValue == null
		 * || newValue.isEmpty()) { return true; }
		 * 
		 * // Compare ANr. String lowerCaseFilter = newValue.toLowerCase();
		 * 
		 * for (int i=0; i <= offersList.size(); i++) { if
		 * (offer.getOfferId().toString().toLowerCase().contains(lowerCaseFilter
		 * )) { controler.filteredData.addAll(offersList); return true; //
		 * Filter matches ANr.. } } return false; // Kein Match });
		 * 
		 * });
		 * 
		 */
	...
	public EventHandler<ActionEvent> FindByDate() {
		 ...
		 * 
		 * //Datum mit Strings vergleichen LocalDate startDate =
		 * controler.getDateStartField().getValue(); LocalDate endDate =
		 * controler.getDateEndField().getValue();
		 * 
		 * Offer date = new Offer();
		 * 
		 * int compareDateFrom =
		 * date.getCreatedDt().compareTo(formatter.format(startDate));
		 * 
		 * if (compareDateFrom >= 0) {
		 * 
		 * int compareDateTo = date.completedDtProperty().compareTo(endDate); if
		 * (compareDateTo <= 0 ) { // hinzufügen der passenden Daten in
		 * filteredData filteredData.addAll(offersList); } }
		 */
		return null;
	}
	...
	public EventHandler<ActionEvent> FindByCustomer() {
		 ...
		 * 
		 * int compCustomer =
		 * customer.getCompanyName().toString().compareTo(cust); if
		 * (compCustomer == 0 ) {
		 * 
		 * // hinzufügen der passenden Daten in filteredData
		 * controler.filteredData.addAll(offersList); } // müsste hier
		 * eigentlich noch wenn buchstaben übereinstimmen auch schon // finden -
		 * nicht nur gesamter String
		 */
		return null;
	 ...
	 * 
	 * for (Offer list : offersList) { int compEmployee =
	 * employee.getLastname().toString().compareTo(emp); if (compEmployee == 0 )
	 * { filteredData.add(offersList); // hinzufügen der passenden Daten in
	 * filteredData filteredData.addAll(offersList); } // müsste hier eigentlich
	 * noch wenn buchstaben übereinstimmen auch schon // finden - nicht nur
	 * gesamter String
	 * 
	 * return null;
	 * 
