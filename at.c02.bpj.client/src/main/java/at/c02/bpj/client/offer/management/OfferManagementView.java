...
public class OfferManagementView implements FxmlView<OfferManagementViewModel>, Initializable {
	...
	public void onExportButtonClick() throws DocumentException, IOException {
		...
		// fileChooser.getExtensionFilters().addAll(new ExtensionFilter("Text
		// Files", "*.txt"),
		// new ExtensionFilter("Image Files", "*.png", "*.jpg", "*.gif"),
		// new ExtensionFilter("Audio Files", "*.wav", "*.mp3", "*.aac"), new
		...
		fileChooser.setInitialFileName(
				"Angebot_" + model.getSelectedOffer().getOfferId().toString() + "_" + df.format(dt));
		...
		if (selectedFile != null) {
			...
			// createPdf("D:\\Campus02\\Offers\\" +
			// model.getSelectedOffer().getOfferId().toString());
			// createPdf("E:\\Offers\\" + "Angebot_" +
			// model.getSelectedOffer().getOfferId().toString() + "_" +
			// df.format(dt));
		}
	...
	public void createPdf(String filename) throws DocumentException, IOException {
		...
		for (Offer offer : Anbot) {
			...
			for (int i = 0; i < offer.getOfferPositions().size(); i++) {
				j++;
				document.add(
						new Paragraph("Artikel " + j + ": " + offer.getOfferPositions().get(i).getArticle().getName()
		...
		document.add(new Paragraph("\n"));
		document.add(new Paragraph(
				"Wir erlauben uns, Ihnen für diese Bestellung " + gesamtpreis + " Euro in Rechnung zu stellen."));
		document.add(new Paragraph("\n"));
		...
		// }
		// for
		// (model.getSelectedOffer().getCustomer().companyNameProperty().toString()
		// : Anbot)
		// {
		// System.out.println(Anbot);
		// }
		...
		document.add(
				new Paragraph("Die Bestellung wurde abgeschlossen am " + model.getSelectedOffer().getCompletedDt()));
		document.add(new Paragraph("\n"));
