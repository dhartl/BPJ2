...
public class OfferManagementView implements FxmlView<OfferManagementViewModel>, Initializable {
	...
	public void createPdf(String filename) throws DocumentException, IOException {
		...
		document.add(new Paragraph(
				"Sehr geehrter Herr " + model.getSelectedOffer().getCustomer().getContactLastName() + ","));
		document.add(new Paragraph("\n"));
		document.add(new Paragraph("wie vereinbart finden Sie untenstehend unser Angebot"));
		document.add(new Paragraph(
				"zu Ihrer Bestellung mit der Nummer " + model.getSelectedOffer().getOfferId().toString() + ":"));
		document.add(new Paragraph("\n\n"));
		document.add(new Paragraph("Auftraggeber:", bold));
		document.add(new Paragraph(model.getSelectedOffer().getCustomer().getContactFirstName() + " "
				+ model.getSelectedOffer().getCustomer().getContactLastName()));
		document.add(new Paragraph(model.getSelectedOffer().getCustomer().getCompanyName()));
		document.add(new Paragraph(model.getSelectedOffer().getCustomer().getStreet() + " "
				+ model.getSelectedOffer().getCustomer().getHouseNr()));
		document.add(new Paragraph(model.getSelectedOffer().getCustomer().getPostCode() + " "
				+ model.getSelectedOffer().getCustomer().getCity()));
		document.add(new Paragraph("\n\n"));

		document.add(new Paragraph("Auftragnehmer:", bold));
		document.add(new Paragraph(model.getSelectedOffer().getEmployee().getFirstname() + " "
				+ model.getSelectedOffer().getEmployee().getLastname()));
		...
		for (Offer offer : Anbot) {
			...
			for (int i = 0; i < offer.getOfferPositions().size(); i++) {
				j = i + 1;
				// document.add(new PdfPCell(table) table.addCell("Artikel " + i
				// + ": " +
				// offer.getOfferPositions().get(i).getArticle().getName()
				// + ", Preis: " +
				// offer.getOfferPositions().get(i).getArticle().getPrice()));
				document.add(new Paragraph(
						"Position " + j + ", Artikel: " + offer.getOfferPositions().get(i).getArticle().getName()
								+ ", Preis: " + offer.getOfferPositions().get(i).getPrice() + ", Anzahl: "
								+ offer.getOfferPositions().get(i).getAmount()));
				// document.add(new Paragraph("Artikel " + j + ": " +
				// offer.getOfferPositions().get(i).getArticle().getName()
				// + ", Preis: " +
				// offer.getOfferPositions().get(i).getArticle().getPrice() + ",
				// Anzahl: " + offer.getOfferPositions().get(i).getAmount()));
				gesamtpreis += offer.getOfferPositions().get(i).getPrice()
						* offer.getOfferPositions().get(i).getAmount();
				// document.add(new Paragraph("Artikel " + i + ": " +
				// offer.getOfferPositions().get(i).getArticle().getName()
				// + ", Preis: " +
				// offer.getOfferPositions().get(i).getArticle().getPrice()));
				// gesamtpreis+=offer.getOfferPositions().get(i).getArticle().getPrice();
				System.out.println("Gesamtpreis: " + gesamtpreis);
				System.out.println(offer.getOfferPositions().get(i).getArticle().getName()
						+ offer.getOfferPositions().get(i).getArticle().getPrice()
						+ offer.getOfferPositions().get(i).getOfferPositionId().toString());
				System.out.println(offer.getOfferPositions().get(i).getArticle().getName()
						+ offer.getOfferPositions().get(i).getArticle().getPrice()
						+ offer.getOfferPositions().get(i).getOfferPositionId().toString());
		...
		document.add(new Paragraph("\n"));
		document.add(new Paragraph(
				"Wir erlauben uns, Ihnen für diese Bestellung " + gesamtpreis + " Euro in Rechnung zu stellen.", bold));
		document.add(new Paragraph("\n\n"));
		document.add(new Paragraph("Vielen Dank für Ihr Vertrauen!"));//
		document.add(new Paragraph("\n\n"));
		// document.add(new Paragraph("Die Bestellung wurde abgeschlossen am " +
		// df.format(dt) + " um " + dg.format(dt) + " Uhr."));
